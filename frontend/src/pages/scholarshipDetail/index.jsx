...
export default function ScholarshipDetail() {
	...
	const scholarship = scholarshipsData.find(item => item.id === Number(id))
	...
	const handleShare = () => {
		if (navigator.share) {
			navigator.share({ title: scholarship.title[currentLang], text: t('scholarships.shareText'), url: window.location.href }).catch(err => console.log('Error sharing:', err))
		} else {
	...
	return (
		<motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className='container mx-auto py-6 md:py-10 px-4 max-w-5xl'>
			...
			<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className='flex justify-end mb-4 md:mb-6'>
				<button onClick={handleShare} className='flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-blue-100 dark:hover:bg-blue-900 text-gray-700 dark:text-gray-300 rounded-lg transition-colors text-sm sm:text-base' aria-label={t('scholarships.share')}>
					<FaShare size={14} />
			...
			<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }} className='mt-8 md:mt-12'>
				<Comments newsId={scholarship.id} />
			</motion.div>
		</motion.section>
	)
}
