...
import { FaArrowLeft, FaCalendarAlt, FaClock, FaDollarSign, FaMapMarkerAlt, FaShare, FaCheckCircle, FaBriefcase } from 'react-icons/fa'
import { Link, useParams } from 'react-router-dom'
import { useTranslation } from 'react-i18next'
import { getInternshipDetail } from '@/api/internships'
...
export default function InternshipDetail() {
	...
	const getTitle = (intern) => {
		if (!intern) return ''
		return intern[`title_${currentLang}`] || intern.title_ru || intern.title_uz || intern.title_en || ''
	}

	// Функция для получения краткого описания на текущем языке
	const getShortDescription = (intern) => {
		if (!intern) return ''
		return intern[`short_description_${currentLang}`] || intern.short_description_ru || intern.short_description_uz || intern.short_description_en || ''
	}

	// Функция для получения контента на текущем языке
	const getContent = (intern) => {
		if (!intern) return ''
		return intern[`content_${currentLang}`] || intern.content_ru || intern.content_uz || intern.content_en || ''
	}

	// Загружаем стажировку с API
	useEffect(() => {
	...
	const handleShare = () => {
		if (navigator.share) {
			navigator.share({ title: getTitle(internship), text: t('internships.shareText'), url: window.location.href }).catch(err => console.log('Error sharing:', err))
		} else {
	...
	return (
		...
		{internship.status === 'active' && (
			<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className='bg-gradient-to-r from-orange-600 to-amber-600 rounded-2xl p-6 md:p-8 text-white mb-8 md:mb-12'>
				...
				<button 
					...
					className='px-6 md:px-8 py-3 md:py-4 bg-white text-orange-600 font-semibold rounded-full hover:bg-gray-100 transition-all duration-300 transform hover:scale-105 shadow-lg text-sm sm:text-base'
				>
		...
		{showApplicationForm && (
			<ApplicationForm
				...
				objectId={internship.id}
				onClose={() => setShowApplicationForm(false)}
				onSuccess={() => {
					setShowApplicationForm(false)
					// Перезагружаем стажировку чтобы обновить счетчик заявок
					getInternshipDetail(id).then(data => setInternship(data))
				}}
			/>
		)}
