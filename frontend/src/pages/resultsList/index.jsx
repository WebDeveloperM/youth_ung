import { resultsData } from '@/datatest/resultsData'
				...
				<div className='container mx-auto'>
					<div className='grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8'>
						<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className='text-center p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md'>
							<div className='text-3xl sm:text-4xl font-bold text-indigo-600 mb-2'>{resultsData.filter(r => r.status === 'completed').length}</div>
						...
						<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className='text-center p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md'>
							<div className='text-3xl sm:text-4xl font-bold text-purple-600 mb-2'>{resultsData.reduce((sum, r) => sum + (r.metrics?.participants || r.metrics?.publications || r.metrics?.treesPlanted || 0), 0)}</div>
						...
						<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className='text-center p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md'>
							<div className='text-3xl sm:text-4xl font-bold text-pink-600 mb-2'>{resultsData.length}</div>
									...
									<div className='md:w-3/5 p-5 md:p-8 flex flex-col justify-between'>
										<div>
											<div className='flex items-center gap-2 mb-2'>
												...
												<span className='text-xs sm:text-sm text-gray-500 dark:text-gray-400'>{result.year} {t('results.year')}</span>
											</div>
											<h2 className='text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-200 mb-3 md:mb-4 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors'>{result.title[currentLang]}</h2>
											<p className='text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-4 md:mb-6 line-clamp-2'>{result.shortDescription[currentLang]}</p>
											
											{/* Metrics */}
											<div className='grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6'>
												{Object.entries(result.metrics).slice(0, 4).map(([key, value], idx) => (
													<div key={idx} className='text-center p-2 bg-gray-50 dark:bg-gray-800 rounded-lg'>
														<div className='text-lg sm:text-xl font-bold text-indigo-600 dark:text-indigo-400 mb-1'>{value}</div>
														<div className='text-xs text-gray-600 dark:text-gray-400'>{t(`results.metrics.${key}`)}</div>
													</div>
												))}
											</div>
										...
										<div className='flex flex-col sm:flex-row gap-3'>
											<Link to={`/results/${result.id}`} className='flex-1 text-center px-5 py-2.5 md:py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 shadow-md text-sm sm:text-base'>
