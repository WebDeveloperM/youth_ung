...
export default function ResearchList() {
	...
	useEffect(() => {
		const loadResearch = async () => {
			try {
				const data = await getResearchList({ is_published: true })
				setResearch(data || [])
			} catch (error) {
				...
				<div className='container mx-auto'>
					<div className='grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8'>
						<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className='text-center p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md'>
							<div className='text-3xl sm:text-4xl font-bold text-green-600 mb-2'>{research.filter(r => r.status === 'active').length}</div>
						...
						<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className='text-center p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md'>
							<div className='text-3xl sm:text-4xl font-bold text-blue-600 mb-2'>{research.reduce((sum, r) => sum + (parseInt(r.publications) || 0), 0)}</div>
						...
						<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className='text-center p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md'>
							<div className='text-3xl sm:text-4xl font-bold text-teal-600 mb-2'>{research.reduce((sum, r) => sum + (parseInt(r.citations) || 0), 0)}</div>
						...
						<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className='text-center p-4 md:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md'>
							<div className='text-3xl sm:text-4xl font-bold text-cyan-600 mb-2'>{research.length}</div>
									...
									<div className='md:w-2/5 relative h-48 md:h-auto overflow-hidden'>
										...
										<div className='absolute bottom-4 right-4'>
											<span className='inline-block px-3 py-1.5 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-full text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-200'>
												{t(`research.category.${item.category}`)}
											</span>
										</div>
									</div>
									<div className='md:w-3/5 p-5 md:p-8 flex flex-col justify-between'>
										<div>
											<h2 className='text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-200 mb-3 md:mb-4 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors'>{item[`title_${currentLang}`] || item.title_ru}</h2>
											...
											<div className='grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-6'>
												...
												<div className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
													...
													<div>
														...
														{item.publications} {t('research.articles')}
													</div>
												</div>
												<div className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
													...
													<div>
														...
														{item.citations} {t('research.times')}
													</div>
												</div>
											</div>
										...
										<div className='flex flex-col sm:flex-row gap-3'>
											<Link to={`/research/${item.id}`} className='flex-1 text-center px-5 py-2.5 md:py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white font-semibold rounded-lg hover:from-green-700 hover:to-teal-700 transition-all duration-300 transform hover:scale-105 shadow-md text-sm sm:text-base'>
