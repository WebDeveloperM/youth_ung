 ...
 * 
 * @param dirty - HTML строка которую нужно очистить
 * @param config - Опциональная кастомная конфигурация DOMPurify
 ...
 * 
 * function NewsDetail({ content }: { content: string }) {
 *   return (
 *     <div 
 ...
 * 
 * @param dirty - HTML строка для очистки
 * @returns Безопасная HTML строка с минимальным форматированием
 */
export const sanitizeStrict = (dirty: string | undefined | null): string => {
 ...
 */
export const stripHTML = (dirty: string | undefined | null): string => {
  if (!dirty) return '';
