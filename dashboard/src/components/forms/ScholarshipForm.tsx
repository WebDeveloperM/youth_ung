...
import { Scholarship, scholarshipsAPI } from '../../api';
...
const ScholarshipForm = ({ scholarship, onClose, onSuccess }: ScholarshipFormProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    category: 'master' as 'master' | 'certification' | 'language' | 'professional',
  });

  useEffect(() => {
    if (scholarship) {
      console.log('📝 ЗАГРУЗКА СТИПЕНДИИ ДЛЯ РЕДАКТИРОВАНИЯ:', scholarship);
      setFormData({
        title_uz: scholarship.title_uz || '',
        title_ru: scholarship.title_ru || '',
        title_en: scholarship.title_en || '',
        short_description_uz: scholarship.short_description_uz || '',
        short_description_ru: scholarship.short_description_ru || '',
        short_description_en: scholarship.short_description_en || '',
        content_uz: scholarship.content_uz || '<p><br></p>',
        content_ru: scholarship.content_ru || '<p><br></p>',
        content_en: scholarship.content_en || '<p><br></p>',
        image: null,
        amount: scholarship.amount || '',
        duration: scholarship.duration || '',
        deadline: scholarship.deadline || new Date().toISOString().split('T')[0],
        status: scholarship.status || 'active',
        category: scholarship.category || 'master',
      });
      if (scholarship.image) {
        setImagePreview(scholarship.image);
      }
    }
  }, [scholarship]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.amount || !formData.duration || !formData.deadline) {
      toast.error('Илтимос, стипендия маълумотларини тўлдиринг!');
      return;
    }

    setLoading(true);
    const loadingToast = toast.loading(scholarship ? 'Сақланмоқда...' : 'Яратилмоқда...');

    try {
      const submitData: any = { ...formData };
      
      console.log('📝 ОТПРАВЛЯЕМ СТИПЕНДИЮ:', submitData);
      ...
      if (scholarship && !formData.image) {
        delete submitData.image;
      }

      if (scholarship) {
        await scholarshipsAPI.update(scholarship.id, submitData);
        toast.success('Стипендия муваффақиятли янгиланди! ✅', { id: loadingToast });
      } else {
        const result = await scholarshipsAPI.create(submitData);
        console.log('✅ СТИПЕНДИЯ СОЗДАНА:', result);
        toast.success('Стипендия муваффақиятли яратилди! 🎉', { id: loadingToast });
      }
      
      onSuccess();
      onClose();
    } catch (error: any) {
      console.error('Ошибка при сохранении стипендии:', error);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white">
            {scholarship ? 'Редактировать стипендию' : 'Новая стипендия'}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Қисқа тавсиф ({activeTab.toUpperCase()})
            </label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Тўлиқ тавсиф ({activeTab.toUpperCase()})
            </label>
